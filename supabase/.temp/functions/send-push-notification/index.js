  ...
  try {
    ...
    if (!user_id || !kind || !payload) {
      return new Response(JSON.stringify({
        error: 'Missing required fields: user_id, kind, payload'
      }), {
        status: 400,
        headers: {
    ...
    const { data: prefs, error: prefsError } = await supabase.from('notification_prefs').select('enabled').eq('user_id', user_id).eq('kind', kind).maybeSingle();
    if (prefsError) {
    ...
    const { data: tokens, error: tokensError } = await supabase.from('user_notification_tokens').select('token').eq('user_id', user_id);
    if (tokensError) {
    ...
    if (invalidTokens.length > 0) {
      await supabase.from('user_notification_tokens').delete().eq('user_id', user_id).in('token', invalidTokens);
